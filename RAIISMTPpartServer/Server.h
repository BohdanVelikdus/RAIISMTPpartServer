...
public:
	Server(const std::string& str, const short port, mSQL::SQLConnection& connection);
	void StartServer();
...
private:
	...
	static PSOCKET_CONNECTED sockets;

	bool UpdateCompletionPort(SOCKET socket, IOOperation operation);
	static DWORD WINAPI WorkerThread(LPVOID WorkThreadContext);
	static VOID CloseClient(PSOCKET_CONNECTED lpSocketConnected, BOOL graceful);
